...
public struct SplunkConfiguration: Sendable {
...
public struct DashboardSyncSettings: Sendable {
...
public struct AppFilters: Sendable {
...
public struct AuthenticationMethods: Sendable {
...
public struct CoreDataMapping: Sendable {
...
public actor SplunkRestClient {

    nonisolated public let configuration: SplunkConfiguration
    private let session: URLSession
    
    public init(configuration: SplunkConfiguration) {
        self.configuration = configuration

        // Configure session for Splunk API with optimized connection pooling
        let config = URLSessionConfiguration.default
        config.timeoutIntervalForRequest = 60  // Longer timeout for search operations
        config.timeoutIntervalForResource = 300

        // Connection pooling optimization
        config.httpMaximumConnectionsPerHost = 4  // Limit concurrent connections
        config.httpShouldSetCookies = false       // API doesn't need cookies
        config.requestCachePolicy = .reloadIgnoringLocalCacheData  // Always fresh data
        config.urlCache = nil  // No response caching for API calls

        // HTTP keep-alive headers for persistent connections
        config.httpAdditionalHeaders = [
            "Connection": "keep-alive",
            "Keep-Alive": "timeout=300, max=100"  // 5 min timeout, 100 requests per connection
        ]
...
public class SplunkDashboardService {
    ...
    public func listDashboards(
        ...
        searchQuery += " | search eai:acl.app=\"\(finalApp)\" AND eai:type=\"views\" |  where NOT match ('eai:data',\"script=.*?js\")  AND isDashboard=1 AND ( like(rootNode,\"dashboard\") OR like(rootNode,\"form\") )  | eval name=title "
        
        //Use to filter system apps/views (TODO: Wire into Settings switch)
        searchQuery += " | search NOT author=\"nobody\" "
...
public actor SplunkSearchService {
...
public struct SplunkAuthResponse: Codable, Sendable {
...
public struct SplunkUser: Codable, Sendable {
...
public struct SplunkCurrentUserResponse: Codable, Sendable {
    public let entry: [SplunkCurrentUserEntry]
    
    public struct SplunkCurrentUserEntry: Codable, Sendable {
...
public struct FormEncodedBody: Codable, @unchecked Sendable {
...
public struct SplunkApp: Codable, Sendable {
...
public struct SplunkAppList: Codable, Sendable {
    public let entry: [SplunkApp]
}

public struct SplunkDashboardList: Codable, Sendable {
    public let entry: [SplunkDashboard]
    public let paging: SplunkPaging
}

public struct SplunkDashboard: Codable, Sendable {
    public let name: String
    public let author: String
    ...
    public struct SplunkDashboardContent: Codable, Sendable {
...
public struct SplunkDashboardResponse: Codable, Sendable {
    public let entry: [SplunkDashboard]
}

public struct SplunkPaging: Codable, Sendable {
    public let total: Int
    public let perPage: Int
    public let offset: Int
}

public struct SplunkSearchJob: Codable, Sendable {
    public let sid: String
}

/// Monitorable search job that can track progress and status
public actor MonitorableSearchJob {
    public nonisolated let sid: String
    private let restClient: SplunkRestClient
...
public struct SplunkSearchJobResponse: Codable, Sendable {
    public let sid: String
}

public struct SplunkSearchJobStatus: Codable, Sendable {
...
public struct SplunkSearchMessage: Codable, Sendable {
...
public struct SplunkSearchJobStatusResponse: Codable, Sendable {
    public let entry: [SplunkSearchJobStatusEntry]
    
    public struct SplunkSearchJobStatusEntry: Codable, Sendable {
...
public struct SplunkSearchResults: Codable, Sendable {
    ...
    private struct RawValue: Codable, @unchecked Sendable {
        let value: Any
